...
class StubLiteLLMClient(LiteLLMClient):
    async def acompletion(
        ...
        **kwargs: Any,
        ...
        spec = stub_completion_spec(messages)
        wants_tool_call = "mock_tool_calls" in spec
        ...
        response = await litellm.acompletion(
            ...
            messages=messages,
            tools=tools,
            stream=False if wants_tool_call else stream,
            **spec,
            **kwargs,
        )
        if not stream or not wants_tool_call:
            return response

        async def single_response() -> Any:
            yield response
